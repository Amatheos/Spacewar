...
struct PlayerStyle {
  Color hull;                      // silhouette + HUD accent
  Color torpedo;                   // torpedo tint
  const std::vector<Vec2>* shape;  // silhouette outline
  Color exhaust;                   // thruster flame tint
  const std::vector<Vec2>* flame;  // thruster flame outline
  float engine;                    // ship-local x of the exhaust port
};

inline const std::array<PlayerStyle, game::kPlayerCount> kPlayerStyles{{

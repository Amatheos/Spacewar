...
void GameView::BuildInto(render::Frame& frame, const game::World& world,
                         bool show_colliders) const {
  const std::array<game::Ship, game::kPlayerCount>& ships = world.ships();
  for (std::size_t i = 0; i < ships.size(); ++i) {
    if (!ships[i].alive()) continue;  // exploded: hull vanishes until respawn

    float timer = world.hyperspace_timer(static_cast<game::Player>(i));
  ...
  const game::Star& star = world.star();
  ...
  for (const game::Torpedo& torpedo : world.torpedoes()) {
    render::DrawCommand cmd;
    cmd.kind = render::DrawCommand::Kind::Circle;
    cmd.pos = torpedo.pos();
  ...
  if (show_colliders) {
    const game::SimSettings& s = world.settings();
    ...
    for (const game::Ship& ship : ships)

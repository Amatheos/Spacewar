...
void Hud::BuildInto(render::Frame& frame, const sim::World& world) const {
  const Vec2 extent = frame.overlay_extent;
  const float cd_max = world.settings().hyperspace.cooldown_sec;
  const std::array<int, sim::kPlayerCount>& scores = world.player_score();
  ...
  for (std::size_t i = 0; i < sim::kPlayerCount; ++i) {
    ...
    float remaining = world.hyperspace_cooldown(static_cast<sim::Player>(i));
